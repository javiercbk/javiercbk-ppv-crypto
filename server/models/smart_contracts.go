...
var SmartContractWhere = struct {
	ID                whereHelperint64
	PayPerViewEventID whereHelperint64
	Address           whereHelperstring
	Currency          whereHelperstring
}{
	ID:                whereHelperint64{field: "\"smart_contracts\".\"id\""},
	PayPerViewEventID: whereHelperint64{field: "\"smart_contracts\".\"pay_per_view_event_id\""},
	Address:           whereHelperstring{field: "\"smart_contracts\".\"address\""},
	Currency:          whereHelperstring{field: "\"smart_contracts\".\"currency\""},
}
...
var (
	smartContractAllColumns            = []string{"id", "pay_per_view_event_id", "address", "currency"}
...
func (smartContractL) LoadPayPerViewEvent(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSmartContract interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`pay_per_view_events`), qm.WhereIn(`pay_per_view_events.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (smartContractL) LoadSmartContractEvents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSmartContract interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`smart_contract_events`), qm.WhereIn(`smart_contract_events.smart_contract_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
