...
var (
	userColumns               = []string{"id", "email", "first_name", "last_name", "password", "registration_date", "created_at", "updated_at", "is_admin"}
...
func (userL) LoadPayments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`payments`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
