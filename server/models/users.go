...
type User struct {
	...
	RegistrationDate time.Time `boil:"registration_date" json:"registrationDate" toml:"registrationDate" yaml:"registrationDate"`
...
var UserWhere = struct {
	...
	RegistrationDate: whereHelpertime_Time{field: "\"users\".\"registration_date\""},
...
var (
	userAllColumns            = []string{"id", "email", "first_name", "last_name", "password", "registration_date", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"email", "first_name", "last_name", "password", "registration_date", "created_at", "updated_at"}
...
func (userL) LoadPayments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`payments`), qm.WhereIn(`payments.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadPermissionsUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`permissions_users`), qm.WhereIn(`permissions_users.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) SetPayments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Payment) error {
	query := "update \"payments\" set \"user_id\" = null where \"user_id\" = $1"
